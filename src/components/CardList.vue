...
function handleSortOrderChange(order) {
  const sortOrder = {
    1: (a, b) => b.rating.rate - a.rating.rate, // Убывание рейтинга
    2: (a, b) => a.rating.rate - b.rating.rate, // Возрастание рейтинга
    3: (a, b) => a.id - b.id, // По умолчанию (по ID)
  };

  cards.value.sort(sortOrder[order]);
}
</script>
